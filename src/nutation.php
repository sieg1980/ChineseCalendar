...
function CalcEarthLongitudeNutation(float $dt) : float
{
    $T = $dt * 10;
    $D = $M = $Mp = $F = $Omega = 0.0;
    ...
    for($i = 0; $i < count(nutation) / count(nutation[0]); $i++)
    {
        $sita = nutation[$i][0] * $D + nutation[$i][1] * $M + nutation[$i][2] * $Mp + nutation[$i][3] * $F + nutation[$i][4] * $Omega;
        $sita = DegreeToRadian($sita);

        $resulte += (nutation[$i][5] + nutation[$i][6] * $T ) * sin($sita);
    }
...
function GetEarthNutationParameter(float $dt, float &$D, float &$M, float &$Mp, float &$F, float &$Omega)
{
